...
class VoiceAnchorManager:
    ...
    def __init__(self, config: Optional[VoiceAnchorConfig] = None):
        ...
        logger.info("VoiceAnchorManager initialized: enabled=%s, threshold=%.2f", 
                    self.config.enabled, self.config.similarity_threshold)
    ...
    def register_character(
        ...
        try:
            ...
            logger.info("Registered voice anchor for '%s': voice=%s, ref=%s", 
                       character_name, voice_id, dest_path)
    ...
    def check_drift(
        ...
        if not Path(anchor.reference_audio_path).exists():
            logger.warning("Reference audio missing for '%s': %s", 
                          character_name, anchor.reference_audio_path)
            return None
        ...
        try:
            ...
            if not result.is_same_speaker:
                ...
                logger.warning(
                    ...
                    character_name, chapter_index, result.similarity, result.threshold
                )
            else:
                logger.debug(
                    "Voice anchor OK for '%s' in chapter %d: similarity=%.3f",
                    character_name, chapter_index, result.similarity
                )
    ...
    def _record_drift_alert(
        ...
        if len(self._drift_alerts[chapter_index]) > self.config.max_drift_alerts_per_chapter:
            logger.error(
                ...
                self.config.max_drift_alerts_per_chapter, chapter_index
            )
...
async def apply_voice_anchor(
    ...
    for inp in inputs:
        ...
        if not manager.has_anchor(char_name):
            ...
            char_binding = next(
                (c for c in inp.character_voice_map 
                 if c.canonical_name == char_name),
                None
            )
            if char_binding:
...
if __name__ == "__main__":
    ...
    print(f"Registered anchor: {anchor}")
    
    # Check if anchor exists
    print(f"Has anchor for narrator: {manager.has_anchor('narrator')}")
    print(f"Reference audio: {manager.get_reference_audio('narrator')}")
