...
DEFAULT_INSTALLED_PATH = Path(__file__).resolve().parents[3] / "config" / "installed_plugins.json"
...
class PluginManifest(BaseModel):
    ...
    def is_loadable(self) -> bool:
        ...
        return bool(self.entry_module) and str(self.type) in {
            PluginType.TTS_ENGINE.value,
            PluginType.LLM_PROVIDER.value,
            PluginType.PIPELINE_STAGE.value,
            # Aliases — ``tts_voice`` maps to the loadable TTS engine type.
            PluginType.TTS_VOICE.value,
        }
    ...
    def entry_module(self) -> str:
        ...
        # Fall back to the legacy ``entry`` field (e.g. ``plugin.py`` or
        # ``plugin``) so manifests that only declare ``entry`` remain loadable.
        if self.entry:
            return self.entry[:-3] if self.entry.endswith(".py") else self.entry
        return ""
