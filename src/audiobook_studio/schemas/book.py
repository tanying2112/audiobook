...
class BookAnalysisInput(BaseModel):
    ...
    target_difficulty: Literal["A", "B", "C", "D"] = Field(
        default="B", description="目标难度等级"
    )
...
class BookMeta(BaseModel):
    ...
    genre: Literal["小说", "散文", "诗歌", "历史", "科普", "童话", "其他"] = Field(
        ..., description="体裁"
    )
...
class CharacterVoiceBinding(BaseModel):
    ...
    age_range: Literal["child", "young", "adult", "elderly", "unknown"] = Field(
        default="unknown", description="年龄段"
    )
    suggested_voice_id: str | None = Field(
        default=None, description="建议声音 ID (TTS 引擎特定)"
    )
...
class BookAnalysisOutput(BaseModel):
    ...
    emotion_snapshots: list[EmotionSnapshot] = Field(
        ..., min_length=1, description="每章情感快照"
    )
    story_line_summary: str = Field(
        ..., min_length=100, max_length=500, description="故事主线摘要 100-500 字"
    )
