...
def check_audio_quality(
    ...
    logger.info(
        f"[AudioMetrics] Quality check for {wav_path}: "
        f"MOS={report.mos:.2f}" if report.mos else "MOS=N/A"
        f", WER={report.wer:.2%}" if report.wer else "WER=N/A"
        f", Cosine={report.speaker_cosine:.3f}" if report.speaker_cosine else "Cosine=N/A"
