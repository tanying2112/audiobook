...
from ..utils.ffmpeg_probe import read_pcm_samples_sync, get_audio_info_sync
...
class DNSMOSMetric(QualityMetric):
    ...
    MODEL_URL = "https://github.com/microsoft/DNS-Challenge/raw/master/DNSMOS/DNSMOS.onnx"
        ...
        try:
            ...
            if not result:
                return ASRResult(
                    ...
                    duration_ms=0, success=False, error="No transcription result"
                )
    ...
    def compute(self, audio_path: Path, reference_text: Optional[str] = None) -> WERResult:
        ...
        if not ref_text:
            return WERResult(
                ...
                success=False, error="No reference text provided"
            )
        ...
        try:
            ...
            if not asr_result.success:
                return WERResult(
                    wer=1.0, cer=1.0, insertions=0, deletions=0, substitutions=0,
                    reference_words=0, hypothesis_words=0,
                    success=False, error=asr_result.error
                )
            ...
            return WERResult(
                wer=wer, cer=cer,
                insertions=ins, deletions=dels, substitutions=subs,
                reference_words=ref_w, hypothesis_words=hyp_w,
                success=True,
            )
        except Exception as e:
    ...
    def _initialize(self):
        ...
        try:
            ...
            self._model = EncoderClassifier.from_hparams(
                ...
                savedir=Path.home() / ".cache" / "audiobook_studio" / "speechbrain" / "ecapa",
            )
            self._initialized = True
    ...
    def _initialize(self):
        ...
        try:
            ...
            self._model = EncoderClassifier.from_hparams(
                ...
                run_opts={"device": self.device},
                savedir=Path.home() / ".cache" / "audiobook_studio" / "speechbrain" / "wavlm",
            )
            self._initialized = True
...
class SpeakerSimilarityMetric(QualityMetric):
    ...
    def register_reference(self, ref_id: str, audio_path: Path) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered reference embedding: {ref_id} (dim={embedding.embedding.shape[0]})")
            return True
        except Exception as e:
    ...
    def compute(
        ...
        if reference_id is None and reference_audio is None:
            return SpeakerSimilarityResult(
                similarity=0.0, threshold=self.threshold, is_same_speaker=False,
                reference_id="", target_id=str(target_audio),
                success=False, error="No reference provided (reference_id or reference_audio)"
            )
        ...
        try:
            ...
            # 计算余弦相似度
            similarity = self._cosine_similarity(ref_embedding.embedding, target_embedding.embedding)
...
class QualityCheckSuite:
    ...
    def check_all(
        ...
        if self._dnsmos and self.config.get("quality_check", {}).get("dnsmos_enabled", True):
            ...
            logger.info(f"DNSMOS: overall={dnsmos_result.mos_ovr:.2f} sig={dnsmos_result.mos_sig:.2f} bak={dnsmos_result.mos_bak:.2f}")
        ...
        if self._speaker_sim and self.config.get("quality_check", {}).get("speaker_similarity_enabled", True):
            ...
            logger.info(f"Speaker Sim: {sim_result.similarity:.3f} (threshold={self.speaker_sim_min}) same={sim_result.is_same_speaker}")
...
if __name__ == "__main__":
    ...
    print("Available checks: DNSMOS, ASR WER, Speaker Similarity")
