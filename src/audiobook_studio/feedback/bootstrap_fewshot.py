    ...
    def __init__(self, prompt_template: str = ""):
        ...
        self.predict = dspy.Predict(
            dspy.Signature(
                ...
                instructions=prompt_template or "Determine the appropriate voice design (style/pacing) for the given character and emotion."
            )
        )
...
if __name__ == "__main__":
    ...
    if result:
        print(f"Optimization complete: improvement={result.improvement_ratio:.2%}")
        print(f"Character accuracy: {result.metrics.character_recognition_accuracy:.2%}")
        print(f"Voice design accuracy: {result.metrics.voice_design_accuracy:.2%}")
        print(f"Budget used: {result.iterations_completed}/{BUDGET_LIMIT}")
