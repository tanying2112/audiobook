...
class HybridRetriever:
    ...
    def __init__(
        ...
        self._bm25_index: Dict[tuple, BM25Okapi] = {}
        self._bm25_doc_ids: Dict[tuple, List[str]] = {}
        self._bm25_documents: Dict[tuple, List[str]] = {}
        self._bm25_tokenized: Dict[tuple, List[List[str]]] = {}
    ...
    def retrieve_context(
        ...
        context.character_profiles = [
            deserialize_character_profile(r.document.metadata) for r in char_results
        ]
        ...
        context.world_building = [
            deserialize_world_building_doc(r.document.metadata) for r in world_results
        ]
        
        # 3. Style guides
        style_results = self.retrieve(query, project_id, DocumentType.STYLE_GUIDE, min(k, 3), strategy)
        context.style_guides = [
            deserialize_style_guide(r.document.metadata) for r in style_results
        ]
        ...
        context.plot_summaries = [
            deserialize_plot_summary(r.document.metadata) for r in plot_results
        ]
        
        # 5. Proper nouns
        noun_results = self.retrieve(query, project_id, DocumentType.PROPER_NOUNS, k * 2, strategy)
        context.proper_nouns = [
            deserialize_proper_nouns(r.document.metadata) for r in noun_results
        ]
...
def init_retriever_from_settings(settings) -> HybridRetriever:
