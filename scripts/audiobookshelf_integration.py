#!/usr/bin/env python3
"""
Audiobook Studio — Audiobookshelf 一键集成
========================================
实现一键将有声书发布到 Audiobookshelf 平台。
"""
...
class AudiobookshelfIntegrator:
    ...
    def prepare_audiobook(
        ...
        if audio_file.format.lower() not in self.supported_formats:
            if self.config.auto_convert:
                ...
                return False, f"不支持的格式 {audio_file.format}，自动转换功能待实现", None
            else:
                return False, f"不支持的格式: {audio_file.format}. 支持的格式: {', '.join(self.supported_formats)}", None
    ...
    def publish_to_audiobookshelf(
        ...
        """
        # 准备数据
        valid, message, upload_data = self.prepare_audiobook(metadata, audio_file)
        if not valid:
            return False, message, None
        ...
        try:
            ...
            if response.get("success"):
                return True, f"有声书已成功发布到 Audiobookshelf (ID: {response.get('book_id')})", response
            else:
                return False, f"发布失败: {response.get('message', '未知错误')}", response
...
def main():
    ...
    # 验证准备工作
    print("\n🔍 验证有声书信息...")
    valid, message, upload_data = integrator.prepare_audiobook(metadata, audio_file)
    if valid:
        print("   ✅ 有声书信息验证通过")
    ...
    print("\n🚀 发布到 Audiobookshelf...")
    success, message, response = integrator.publish_to_audiobookshelf(metadata, audio_file)
    ...
    # 显示库状态
    print("\n📊 Audiobookshelf 库状态:")
    status = integrator.get_library_status()
