...
class EmotionSnapshot(BaseModel):
    ...
    dominant_emotion: Literal[
        ...
        "disgusted", "tense", "tender", "contemplative",
...
def create_annotations(chapter_idx: int, paragraphs: List[str]) -> List[ParagraphAnnotation]:
    ...
    for i, text in enumerate(paragraphs):
        ...
        annotations.append(ParagraphAnnotation(
            paragraph_index=i,
            text=text[:2000],
...
def main():
    ...
    for ch_idx, ch_data in enumerate(chapters_data, 1):
        ...
        emotion_snapshots = [
            EmotionSnapshot(chapter=1, dominant_emotion="contemplative", intensity=0.5, notes="王子诞生与成长"),
            EmotionSnapshot(chapter=2, dominant_emotion="tense", intensity=0.8, notes="多重危机爆发"),
            EmotionSnapshot(chapter=3, dominant_emotion="tense", intensity=0.7, notes="宫廷内部动荡与谈判"),
            EmotionSnapshot(chapter=4, dominant_emotion="contemplative", intensity=0.6, notes="雪狼部落和谈策略"),
            EmotionSnapshot(chapter=5, dominant_emotion="contemplative", intensity=0.6, notes="海盗问题根源分析"),
            EmotionSnapshot(chapter=6, dominant_emotion="happy", intensity=0.7, notes="农业改革成果"),
            EmotionSnapshot(chapter=7, dominant_emotion="tender", intensity=0.6, notes="文化建设与智慧传承"),
        ]
        ...
        ch_source = ChapterSource(
            ...
            emotion_snapshot=emotion_snapshots[ch_idx - 1] if ch_idx <= len(emotion_snapshots) else emotion_snapshots[-1],
