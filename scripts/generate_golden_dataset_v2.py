...
EXTRACT_SAMPLES = [
    {
        "input": {"file_path": f"/data/samples/book_{i}.pdf", "mime_type": "application/pdf", "detect_language": True},
        "expected_output": {
...
EDIT_SAMPLES = [
    {
        ...
        "expected_output": {
            "edited_text": (
                f"这是一个测试段落。编号 {i+1}。"
                if i % 10 != 0
                else f"这是一个很长的测试段落，包含多个句子。" * 3 + f" 编号 {i+1}。"
            ),
            "changes_made": (
                ["长句拆分", "去除冗余"]
                if i % 10 != 0 and i % 4 >= 2
                else ["保持原文"] if i % 10 == 0 else ["标点调整"]
            ),
...
TRANSLATE_SAMPLES = [
    {
        ...
        "expected_output": {
            "translated_text": (
                f"This is test translation {i+1}." if i % 5 == 0 else f"这是第 {i+1} 个翻译测试的源文本内容。"
            ),
            "target_language": ["en-US", "es-ES", "fr-FR", "ja-JP", "ko-KR"][i % 5],
            "voice_id": [
                "en-US-Neural2-J",
            ...
            "prosody_overrides": {"rate": str(round(0.8 + (i % 5) * 0.1, 1)), "pitch": f"{((i % 5) - 2) * 1}st"},
        ...
        "expected_output": {
            ...
            "fix_suggestions": (
                ...
                if i % 3 != 0
                    ...
                    {
                        "suggestion_type": "emotion_adjustment",
                        "target_text": "测试文本",
                        "suggested_value": "增加情感强度",
                        "confidence": 0.8,
                        "rationale": "情感表达不足",
                        "priority": "high",
                    }
            ...
            "judgment": {
                ...
                "fix_suggestions": (
                    ...
                    if i % 4 != 0
                        ...
                        {
                            "suggestion_type": "pacing_adjustment",
                            "target_text": "测试文本",
                            "suggested_value": "调整语速",
                            "confidence": 0.85,
                            "rationale": "语速过快",
                            "priority": "medium",
                        }
...
def generate_dataset():
    ...
    for stage_name, samples in STAGES.items():
        ...
        splits = {"train": samples[:train_end], "val": samples[train_end:val_end], "test": samples[val_end:]}
