...
class TestMainWithLLMData:
    def test_check_only(self, tmp_path):
        ...
        records = [{"timestamp": now, "schema_compliance": True, "model": "gpt-4", "cost_usd": 0.01}] * 5
        _write_perf_log(tmp_path, records)
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path), "--check-only", "--hours", "1"]):
            main()

    def test_with_alerts_no_webhook(self, tmp_path):
        now = datetime.now().isoformat()
        records = [{"timestamp": now, "schema_compliance": False, "model": "fallback-x", "cost_usd": 0.01}] * 10
        _write_perf_log(tmp_path, records)
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path), "--check-only"]):
            main()
    ...
    def test_send_alerts_dingtalk(self, tmp_path):
        now = datetime.now().isoformat()
        records = [{"timestamp": now, "schema_compliance": False, "model": "gpt-4", "cost_usd": 0.01}] * 10
        _write_perf_log(tmp_path, records)
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path), "--dingtalk-webhook", "http://hook.d"]):
            with patch("src.audiobook_studio.monitoring.alert.send_dingtalk_alert", return_value=True):
                main()
    ...
    def test_send_alerts_slack(self, tmp_path):
        ...
        records = [{"timestamp": now, "schema_compliance": False, "model": "gpt-4", "cost_usd": 0.01}] * 10
        _write_perf_log(tmp_path, records)
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path), "--slack-webhook", "http://hook.s"]):
            with patch("src.audiobook_studio.monitoring.alert.send_slack_alert", return_value=True):
                main()
    ...
    def test_send_alerts_both_fail(self, tmp_path):
        ...
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path),
                                "--dingtalk-webhook", "http://d", "--slack-webhook", "http://s"]):
            with patch("src.audiobook_studio.monitoring.alert.send_dingtalk_alert", return_value=False):
                with patch("src.audiobook_studio.monitoring.alert.send_slack_alert", return_value=False):
                    main()
...
class TestMainCombined:
    def test_both_data_sources(self, tmp_path):
        now = datetime.now().isoformat()
        perf_records = [{"timestamp": now, "schema_compliance": True, "model": "gpt-4", "cost_usd": 0.01}] * 3
    ...
    def test_env_webhooks(self, tmp_path):
        now = datetime.now().isoformat()
        records = [{"timestamp": now, "schema_compliance": False, "model": "gpt-4", "cost_usd": 1.0}] * 20
        _write_perf_log(tmp_path, records)
        with patch("sys.argv", ["alert.py", "--logs-dir", str(tmp_path)]):
            with patch.dict(os.environ, {"DINGTALK_WEBHOOK": "http://env-d", "SLACK_WEBHOOK": "http://env-s"}):
                with patch("src.audiobook_studio.monitoring.alert.send_dingtalk_alert", return_value=True):
                    with patch("src.audiobook_studio.monitoring.alert.send_slack_alert", return_value=True):
                        main()
