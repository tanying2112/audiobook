...
class TestExtractAgent:
    ...
    def test_handle_message_success(self):
        ...
        with patch(
            ...
            "src.audiobook_studio.pipeline.extract.extract_text"
        ) as mock_extract:
...
class TestAnalyzeAgent:
    ...
    def test_handle_message_success(self):
        ...
        with patch("src.audiobook_studio.pipeline.agents.get_db") as mock_get_db, patch(
            "src.audiobook_studio.pipeline.analyze_structure.analyze_structure"
        ) as mock_analyze:
            ...
            assert mod_task_record.status == "COMPLETED"
            assert mod_task_record.output_data == mod_analyze_result.model_dump()
            assert mod_task_record.completed_at is not None
            mod_session.commit.assert_called_once()
            # get_db session is not closed by agent (see code)
            mod_session.close.assert_not_called()
            agent._handle_failure.assert_not_called()
    ...
    def test_handle_message_exception(self):
        ...
        with patch("src.audiobook_studio.pipeline.agents.get_db") as mock_get_db, patch(
            ...
            mod_session.query.return_value.filter_by.return_value.first.return_value = (
                mod_task_record
            )
            mod_analyze.side_effect = Exception("analyze failed")
            message = MagicMock()
            message.content = {
                "raw_text": "test",
                "title_hint": "title",
            ...
            agent._handle_message(message)
            agent._handle_failure.assert_called_once()
            mod_session.commit.assert_not_called()
...
class TestSynthesizeAgent:
    ...
    def test_handle_message_success(self):
        ...
        mock_session = MagicMock()
        mod_task4 = MagicMock(spec=TaskRecord)
        mock_segment = MagicMock(spec=AudioSegment)
        mod_segment.to_dict.return_value = {"dummy": "segment"}
        with patch(
            "src.audiobook_studio.pipeline.agents.get_db"
        ) as mock_get_db, patch.object(agent, "pipeline") as mod_pipeline:
            mod_get_db.return_value = iter([mod_session])
            mod_session.query.return_value.filter_by.return_value.first.return_value = (
                mod_task4
            )
            mod_pipeline.run.return_value = [mod_segment]
            message = MagicMod()
            message.content = {
                "text": "test text",
            ...
            agent._handle_message(message)
            mod_pipeline.run.assert_called_once_with(
                text="test text",
                voice_params={"voice_id": "test_voice"},
                quality_level="standard",
            )
            assert mod_task4.status == "COMPLETED"
            assert mod_task4.output_data is not None
            assert "audio_segments" in mod_task4.output_data
            assert len(mod_task4.output_data["audio_segments"]) == 1
            assert mod_task4.output_data["audio_segments"][0] == {"dummy": "segment"}
            assert mod_task4.output_data["book_id"] == 1
            assert mod_task4.completed_at is not None
            mod_session.commit.assert_called_once()
            mod_session.close.assert_not_called()
            agent._handle_failure.assert_not_called()
    ...
    def test_handle_message_exception(self):
        ...
        with patch(
            "src.audiobook_studio.pipeline.agents.get_db"
        ) as mod_get_db, patch.object(agent, "pipeline") as mod_pipeline:
            mod_get_db.return_value = iter([mod_session])
            mod_session.query.return_value.filter_by.return_value.first.return_value = (
                mod_task4
            )
            mod_pipeline.run.side_effect = Exception("synth failed")
            message = MagicMod()
...
class TestQualityAgent:
    ...
    def test_handle_message_success(self):
        ...
        mock_session = MagicMock()
        mod_task4 = MagicMock(spec=TaskRecord)
        mod_judgment = MagicMock(spec=QualityJudgment)
        mod_judgment.model_dump.return_value = {"dummy": "judgment"}
        with patch(
            "src.audiobook_studio.pipeline.agents.get_db"
        ) as mock_get_db, patch.object(agent, "pipeline") as mod_pipeline:
            mod_get_db.return_value = iter([mod_session])
            ...
            mod_pipeline.run.return_value = mod_judgment
            message = MagicMod()
            message.content = {
            ...
            assert mod_task4.status == "COMPLETED"
            assert mod_task4.output_data == {"dummy": "judgment"}
            assert mod_task4.completed_at is not None
            mod_session.commit.assert_called_once()
            mod_session.close.assert_not_called()
            agent._handle_failure.assert_not_called()
    ...
    def test_handle_message_exception(self):
        ...
        with patch(
            ...
            mod_pipeline.run.side_effect = Exception("quality failed")
            message = MagicMod()
            message.content = {
                "audio_segments": [],
                "reference_text": "ref",
