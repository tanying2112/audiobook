...
class TestWERResult:
    def test_to_dict(self):
        ...
        r = WERResult(wer=0.1, cer=0.05, insertions=1, deletions=0, substitutions=1, reference_words=10, hypothesis_words=11, success=True)
        assert r.to_dict()["wer"] == 0.1

class TestSpeakerEmbedding:
    def test_to_dict(self):
        from src.audiobook_studio.quality.metrics import SpeakerEmbedding
        emb = SpeakerEmbedding(embedding=np.array([0.1, 0.2, 0.3], dtype=np.float32), model_name="ecapa", sample_rate=16000)
...
class TestSpeakerSimilarityResult:
    def test_to_dict(self):
        ...
        r = SpeakerSimilarityResult(similarity=0.85, threshold=0.7, is_same_speaker=True, reference_id="r", target_id="t", success=True)
