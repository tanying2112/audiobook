...
class TestAudiobookshelfIntegratorExtended:
    ...
    def test_prepare_upload_data(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            af = AudiobookFile(
                file_path=fp, size_bytes=5,
                duration_seconds=3600, format="m4b", bitrate_kbps=64, checksum_md5="x",
            )
    ...
    def test_dataclass_defaults(self):
        """数据类默认值。"""
        from src.audiobook_studio.publish.audiobookshelf_integration import AudiobookshelfConfig, AudiobookMetadata
    ...
    def test_audiobook_file_defaults(self):
        ...
        from src.audiobook_studio.publish.audiobookshelf_integration import AudiobookFile
        f = AudiobookFile(
            file_path=Path("/x.m4b"), size_bytes=100,
            duration_seconds=60, format="m4b", bitrate_kbps=64, checksum_md5="x",
        )
        assert f.chapters == []
