...
class TestRAGGoldenDataset:
    ...
    def _populate_golden_data(cls):
        ...
        for key, wb_data in cls.golden["world_building"].items():
        ...
        for tc in self.test_cases["proper_noun_drift_tests"]:
            ...
            for r in results[:3]:  # Top 3 results
                for forbidden_variant in forbidden:
                    assert forbidden_variant not in r.document.content, \
    ...
    def test_hybrid_search_strategy(self):
        ...
        semantic_results = self.retriever.retrieve(
            query, project_id, DocumentType.PROPER_NOUNS, n_results=5, 
            strategy="semantic"
        )
    ...
    def test_no_character_name_drift(self):
        ...
        canonical_names = [
            "艾琳·星织", "凯尔·霜誓", "赛琳娜·炎心", "索恩·深岩", "莉拉·青藤"
        ]
        
        for canonical in canonical_names:
            results = self.retriever.retrieve(
                canonical, project_id, DocumentType.CHARACTER_PROFILE, n_results=3
            )
    ...
    def test_magic_system_consistency(self):
        ...
        for query in queries:
            results = self.retriever.retrieve(
                query, project_id, DocumentType.WORLD_BUILDING, n_results=3,
                where={"doc_type": "magic_system"}
            )
...
class TestRAGSchemaCompatibility:
    ...
    def test_rag_context_model_structure(self):
        ...
        from audiobook_studio.rag.models import RAGContext, CharacterProfile
