...
class TestBenchVoxcpm2:
    ...
    def test_compute_voxcpm2_projection(self):
        ...
        from src.audiobook_studio.benchmarks.bench_voxcpm2 import compute_voxcpm2_projection, HardwareProfile
        hw = HardwareProfile()
    ...
    def test_build_summary(self):
        ...
        from src.audiobook_studio.benchmarks.bench_voxcpm2 import (
            build_summary, HardwareProfile, VoxCPM2Projection, TtsBenchmarkResult
        )
        hw = HardwareProfile()
        proj = VoxCPM2Projection()
        tts_results = [TtsBenchmarkResult(engine="test", text_length_chars=100, 
                                          audio_duration_sec=20.0, synthesis_time_sec=4.0, 
                                          rtf=0.2, throughput_cps=25.0, success=True)]
    ...
    def test_build_recommendations(self):
        ...
        from src.audiobook_studio.benchmarks.bench_voxcpm2 import (
            build_recommendations, HardwareProfile, VoxCPM2Projection
        )
        hw = HardwareProfile()
        proj = VoxCPM2Projection()
    ...
    def test_build_acceptance_criteria(self):
        ...
        from src.audiobook_studio.benchmarks.bench_voxcpm2 import (
            build_acceptance_criteria, HardwareProfile, VoxCPM2Projection, TtsBenchmarkResult
        )
        hw = HardwareProfile()
        proj = VoxCPM2Projection()
    ...
    def test_render_markdown_report(self):
        ...
        from src.audiobook_studio.benchmarks.bench_voxcpm2 import (
            render_markdown_report, BenchmarkReport, HardwareProfile, 
            VoxCPM2Projection, TtsBenchmarkResult
        )
        hw = HardwareProfile()
        proj = VoxCPM2Projection()
        tts_results = [TtsBenchmarkResult(engine="test", success=True)]
        report = BenchmarkReport(
            hardware=hw, voxcpm2_projection=proj, 
            edge_tts_results=tts_results,
            acceptance_criteria_met={"test": True}
        )
