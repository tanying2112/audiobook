...
# Force MOCK_LLM=true for this E2E verification script — it uses mock mode
# for all pipeline stages and does not require real API keys.
...
def check_env_keys():
    """Check and report status of critical environment variables."""
    # Langfuse keys
    ...
    # LLM provider keys (at least one should be set for non-mock mode)
    llm_keys = {
    ...
    llm_available = any(v for v in llm_keys.values())
    ...
    if mock_mode:
        ...
        print("   LLM keys: ❌ NONE configured — non-mock LLM calls will fail!")

    return langfuse_ok, llm_available, mock_mode
...
def main():
    ...
    # Verify environment keys before running
    check_env_keys()
